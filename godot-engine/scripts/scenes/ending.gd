...
func _load_ending() -> void:
	...
	ending_data = {
		...
		"collected_clues": progress.get("collected_clues", []).size(),
...
func _update_stats(progress: Dictionary) -> void:
	var play_time_min = int(progress.get("play_time", 0.0) / 60)
	var play_time_sec = int(progress.get("play_time", 0.0)) % 60

	var stats_text = """对话次数：%d
关键线索：%d/%d
用时：%02d 分 %02d 秒""" % [
		StoryManager.stats.get("total_dialogues", 0),
		progress.get("collected_clues", []).size(),
